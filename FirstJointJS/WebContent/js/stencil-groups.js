(function() {
	...
	// Show Inspector immediately for the rectangle so that it is visible to the
	...
	function buildGraphFromAdjacencyList(adjacencyList) {
		...
		// Links must be added after all the elements. This is because when the
		// links
	...
	$('#btn-layout2').on('click', layout);

	function layout() {

		try {
			var adjacencyList = eval('adjacencyList = '
					+ $('#adjacency-list').val());
		} catch (e) {
			alert(e);
		}
